// src/views/MindView.jsx
// Mind domain tab — meditation timer, breathwork patterns, daily gratitude,
// focus areas, nootropic stack (Pro), goals, today's tasks.
//
// Ported from v2-revival-archive:src/views/MindView.jsx (Phase 4 Task 9).
// Sanctioned adaptations:
//   1. The archive embedded its own BREATHWORK_PATTERNS / NOOTROPICS /
//      FOCUS_LABELS consts inline. All three are deleted here and imported
//      from the single-sourced src/protocols/mind/data.js (landed in Task 1).
//   2. Meditation completion used to be a lie: the timer hit 0, flashed a
//      "Session logged" toast, and wrote nothing anywhere. Now, when the
//      timer completes naturally (counts down to 0 on its own — not on
//      "End early"), MeditationCard also calls
//      `setProtocolState('mind', { lastMeditation: <iso> })` so the toast's
//      claim is backed by real state. "End early" still resets the timer and
//      flashes the toast (matches the archive's UX) but does not record a
//      completion, since the session wasn't actually finished.
//   3. NootropicsCard's archive gate was cosmetic — a static "Pro" tag with
//      no actual access check; the card only rendered at all when the user's
//      onboarding answer `mind.nootropicsOpen` was true, with zero
//      enforcement inside. That visibility gate is kept verbatim (the card
//      only appears once the user has opted in during onboarding), but the
//      card now ALSO gates its *content* on `profile.tier`: free tier
//      renders the header + a locked-chrome upsell (no interactive toggles),
//      pro/elite tiers render the full 8-compound interactive stack — same
//      tier check style as `src/protocols/money/credit/index.js`
...
import { BREATHWORK_PATTERNS, NOOTROPICS, FOCUS_LABELS } from '../protocols/mind/data';
...
function MeditationCard({ setProtocolState }) {
      ...
      intervalRef.current = setInterval(() => {
        setSecondsLeft(prev => {
          if (prev <= 1) {
            ...
            // v2: archive's "Session logged" toast wrote nothing — now the
            // natural (non-early-exit) completion is recorded.
            if (setProtocolState) {
              setProtocolState('mind', { lastMeditation: new Date().toISOString() });
            }
            return 600;
          }
          return prev - 1;
  ...
  }, [running, paused, setProtocolState]);
    ...
    <>
      <div style={{ ...s.card, marginBottom: 12 }}>
        ...
        <div style={{ fontSize: 12, color: P.txD, marginBottom: 12 }}>
          {BREATHWORK_PATTERNS.length} patterns. Pick by intent.
        </div>
...
function NootropicsCard({ nootropicsActive, onToggle, tier }) {
  const active = nootropicsActive || {};
  const unlocked = tier === 'pro' || tier === 'elite';
  ...
  return (
    <div style={{ ...s.card, marginBottom: 12 }}>
      ...
      {!unlocked ? (
        // Locked chrome: visible so free-tier users know the feature exists,
        // but no interactive toggles and no compound-level detail.
        <div>
          <div style={{ fontSize: 12, color: P.txD, marginBottom: 12 }}>
            {NOOTROPICS.length} research-backed compounds. Upgrade to Pro to unlock the stack.
          </div>
          <div style={{
            ...
            <div style={{ fontSize: 10, color: P.txD, marginTop: 4, lineHeight: 1.5 }}>
              Caffeine + L-Theanine, Lion's Mane, Creatine, and 5 more — with dosing and timing.
            </div>
          </div>
        </div>
      ) : (
...
export default function MindView({
  ...
  return (
    <div>
      ...
      <MeditationCard setProtocolState={setProtocolState} />
      <BreathworkCard />
      <GratitudeCard
