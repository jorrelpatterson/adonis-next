...
//
// source: v2-revival-archive:src/views/components/WeeklyRecap.jsx
// Two sanctioned bug fixes applied on port (see inline `archive bug fixed on
// port:` comments below) — the archive shipped this component with zero
// tests, which is how both bugs survived undetected.
...
import { calcCalorieTarget } from '../../protocols/body/nutrition/calorie-engine';
 ...
 *
 * `goal` is optional and not part of the archive's signature — the archive's
 * own caller (RoutineView.jsx) never passed one, instead deriving it from
 * `profile?.primary` (same pattern used by HomeDashboard.jsx and
 * FoodLogger.jsx: `profile?.primary || 'Wellness'`). We follow that same
 * convention here as the default, so existing verbatim callers keep working
 * unchanged, while still accepting an explicit `goal` override for testing
 * or future callers that already know the active goal.
 */
export function buildWeekStats({ logs, profile, today = ymd(new Date()), goal }) {
  const start = shiftDay(today, -6);
  const week = Array.from({ length: 7 }, (_, i) => shiftDay(start, i));
  const effectiveGoal = goal || profile?.primary || 'Wellness';
  ...
  const caloriesByDay = week.map(d => {
    const meals = food[d] || [];
    // archive bug fixed on port: summed `m.cals`, but FoodLogger writes `m.cal`
    // (singular) — every day totaled 0, so `c > 0` below always excluded it.
    return meals.reduce((sum, m) => sum + (Number(m.cal) || 0), 0);
  });
  // archive bug fixed on port: read `profile.targetCal`, a field that exists
