...
// Mifflin-St Jeor BMR. Inputs are imperial: lbs, ft, in, years.
// Returns 0 for missing/invalid inputs so the UI can fall back gracefully.
...
// Final daily calorie target = TDEE + goal adjustment.
// Returns 0 if profile too sparse to compute.
...
// Sum totals across an array of meals: { cal, p, c, f }.
...
export default function FoodLogger({ profile, protocolStates, logs, log }) {
  ...
  const target = useMemo(() => calcCalorieTarget(profile, goal), [profile, goal]);
