...
//
// Ported from v2-revival-archive:src/views/ImageView.jsx (Phase 4 Task 8).
// Sanctioned adaptations (see .superpowers/sdd/task-8-report.md):
//   1. Grooming: the archive's local `GROOMING_SEED` (6 items incl.
//      body-hair/teeth-whitening, field `cadence`) is deleted. Grooming now
//      renders `GROOMING_ITEMS` from src/protocols/image/skincare/data.js —
//      the CONTROLLER-RULED canonical item set (Task 2): ids
//      haircut/beard/nails/dental/brows/nose_ear, field `freqDays` (not
//      `cadence`). The archive-only `body`/`teeth` items do NOT ride in.
//      `item.freqLabel` (a pre-formatted string like "Every 3 weeks") now
//      drives the row's frequency copy instead of the archive's hardcoded
//      "Every N days"; `item.freqDays` still drives the days-since/ratio math.
//   2. Wardrobe: the archive's local `WARDROBE` (hardcoded non-zero `have`
//      counts, no interaction) is deleted. Wardrobe now reads
//      `protocolStates.skincare?.wardrobe ?? WARDROBE` (WARDROBE imported
//      from the same skincare data.js, `have` starts at 0 per Task 2). The
//      archive never actually had an edit mechanism for wardrobe counts (it
//      was pure display) despite the brief's expectation of one to "keep" —
//      confirmed by inspection. Since Task 2 zeroed every `have` count,
//      leaving the section permanently stuck at 0/N would be a functional
//      regression, so this port adds a minimal +/- control per category,
//      persisted via `setProtocolState('skincare', { wardrobe: next })` —
//      the same store slice the archive's grooming mechanism already uses.
...
export default function ImageView({
  ...
  // Persisted wardrobe counts — falls back to the zeroed default set.
  const wardrobe = Array.isArray(skin.wardrobe) ? skin.wardrobe : WARDROBE;
  ...
  const adjustWardrobe = (id, delta) => {
    if (!setProtocolState) return;
    const next = wardrobe.map((cat) =>
      cat.id === id ? { ...cat, have: Math.max(0, cat.have + delta) } : cat
    );
    setProtocolState('skincare', { wardrobe: next });
  };
  ...
  return (
    <div>
      ...
      <div style={{ ...s.card, padding: 16, marginBottom: 12 }}>
        ...
        {GROOMING_ITEMS.map((item, i) => {
          const lastISO = groomingLastDone[item.id];
          const days = daysSince(lastISO, todayISO);
          const ratio = days === null ? 1 : Math.min(1, days / item.freqDays);
          let barColor = P.ok;
          ...
          return (
            <div key={item.id} style={{
              padding: '10px 0', borderBottom: i < GROOMING_ITEMS.length - 1 ? '1px solid ' + P.bd : 'none',
            }}>
      ...
      <div style={{ ...s.card, padding: 16, marginBottom: 12 }}>
        ...
        <div style={{ height: 4, borderRadius: 2, background: 'rgba(232,213,183,0.08)', marginBottom: 14 }}>
          <div style={{
            height: '100%', borderRadius: 2,
            width: (wardrobeTotal > 0 ? (wardrobeHave / wardrobeTotal) * 100 : 0) + '%',
          ...
          return (
            <div key={cat.id} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <button
                  onClick={() => adjustWardrobe(cat.id, -1)}
                  aria-label={'Decrease ' + cat.name}
                  style={{
                    width: 18, height: 18, borderRadius: 5, flexShrink: 0,
                    border: '1px solid ' + P.gW + '33', background: 'transparent',
                    color: P.gW, fontFamily: FN, fontSize: 11, fontWeight: 700,
                ...
                <span style={{
                  ...
                  color: complete ? P.ok : P.gW, minWidth: 34, textAlign: 'center',
                }}>
                  {cat.have}/{cat.target}
                </span>
                <button
