// src/routine/RoutineView.jsx
// Time-block calendar + intelligence surfaces for the daily routine.
//
// source: v2-revival-archive:src/routine/RoutineView.jsx
// adapted (documented, per task-14 brief):
//  1. Dropped the archive's embedded `<HomeDashboard>` render (was gated on
//     `isToday`, top of the tree). Task 13 already promoted HomeDashboard to
//     its own first-class "Home" tab in App.jsx (`activeTab === 'home'`),
//     wired with its own `onCheckinTap`. Keeping the archive's inline embed
//     here would double-render HomeDashboard (once as the Home tab, again
//     at the top of Routine when viewing today) and duplicate the check-in
//     tap wiring. The `HomeDashboard` import was removed along with it. This
//     is the one rendered section from the archive intentionally NOT ported
//     — task-14-report.md documents it explicitly. Everything else the
//     brief's interface list calls for (pace banner, yesterday recap, alert
//     stack, goal cards, time-block calendar, long-press context menu,
//     Sunday recap, deferred/upsell/retention) is verbatim.
//  2. `data-testid={`day-chip-${i}`}` re-added to the day-selector buttons
//     — a5d7782 (main, pre-Task-14) added these hooks to the old flat
//     RoutineView so App.test.jsx / the Task-13 day-navigation regression
//     test could click a specific day chip unambiguously. The archive's
//     premium day-selector markup didn't have them; preserved here per the
//     task-14 brief's contract-preservation instruction.
//  3. All imports otherwise point at the same relative paths the archive
//     used — every dependency (group-by-time, intelligence, streak,
//     WeeklyRecap, StreakMilestone, ProgressBar, haptics/sound/useLongPress,
//     TaskContextMenu, ExerciseDetail, adaptive-calories) already landed on
//     main at those exact paths, so no repoints were needed here.
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: 6, marginBottom: 16 }}>
        {DS.map((label, i) => {
          ...
          return (
            <button key={i} data-testid={`day-chip-${i}`} onClick={() => {
