...
//
// Ported from v2-revival-archive:src/app/components/ProfileHeader.jsx.
// Task 13 adaptations (see task-13-report.md for the full list):
//   1. Import repoint — archive imported calcCalorieTarget from a local
//      ./protocols/body/nutrition/math shim that doesn't exist on main;
//      repointed to ../../protocols/body/nutrition/calorie-engine (same
//      repoint Task 13/phase3's HomeDashboard.jsx already made for main).
//   2. Sentinel-discipline guard — calcCalorieTarget returns the literal
//      integer 0 as its "profile too sparse to compute" sentinel (see that
//      file's own doc comment). The archive's unconditional
//      `{ label: 'Target Cal', numeric: targetCal, ... }` would render "0"
//      for an incomplete profile instead of omitting the stat. The 'Target
//      Cal' stat is now only pushed onto the grid when targetCal > 0.
...
export default function ProfileHeader({
  ...
  // Sentinel discipline: calcCalorieTarget's 0 return means "profile too
  // sparse" — never surface that as a displayed stat (see file header note).
  if (targetCal > 0) {
    stats.push({ label: 'Target Cal', numeric: targetCal, format: (n) => Math.round(n).toLocaleString() });
  }
  ...
  return (
    <>
      {/* Summary card */}
      <div style={{ ...s.card, padding: 16, marginBottom: 12 }} data-testid="profile-header">
