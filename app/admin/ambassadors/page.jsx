...
export default function AmbassadorsPage() {
  ...
  const getEarnings = (ambId) => ({
    l1: commissions.filter(c => c.l1_ambassador_id === ambId).reduce((s,c) => s + parseFloat(c.l1_amount||0), 0),
    l2: commissions.filter(c => c.l2_ambassador_id === ambId).reduce((s,c) => s + parseFloat(c.l2_amount||0), 0),
    l3: commissions.filter(c => c.l3_ambassador_id === ambId).reduce((s,c) => s + parseFloat(c.l3_amount||0), 0),
  });

  const getComms = (ambId) => commissions.filter(c =>
  ...
  const sendPayout = async (amb) => {
    ...
    try {
      const res = await fetch('/api/ambassador-payout', {
        ...
        body: JSON.stringify({ ambassador: { name:amb.name, email:amb.email, code:amb.code, period, l1_amount:l1, l2_amount:l2, l3_amount:l3 }})
      });
                ...
                <div style={{padding:'0 20px 20px',borderTop:'1px solid #F0F1F4'}}>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:24,marginTop:16}}>
                    <div>
                      ...
                      {[{l:'Direct sales (L1)',v:earn.l1},{l:'Recruit sales (L2)',v:earn.l2},{l:'L3 overrides',v:earn.l3}].map((x,i)=>(
                    ...
                    <div>
                      ...
                      <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:12}}>
                        {[{key:'l1',label:'L1 amount ($)'},{key:'l2',label:'L2 amount ($)'},{key:'l3',label:'L3 amount ($)'}].map(f=>(
                          <div key={f.key} style={{display:'flex',gap:8,alignItems:'center'}}>
                            ...
                            <input style={{...cs.input,width:100}} type="number" step="0.01" min="0" placeholder="0.00"
                              ...
                              onChange={e=>setPayout(prev=>({...prev,[amb.id]:{...(prev[amb.id]||{}),[f.key]:e.target.value}}))} />
                          </div>
                        ))}
                      </div>
                      ...
                      <button onClick={()=>sendPayout(amb)} disabled={sending[amb.id]}
