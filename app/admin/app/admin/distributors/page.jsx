...
export default function DistributorsPage() {
  ...
  const approve = async (dist) => {
    ...
    const { error } = await supabase.from('distributors')
      .update({
        status:      'approved',
        tier:        form.tier || 'entry',
        login_code:  form.login_code.toUpperCase(),
        approved_at: new Date().toISOString(),
      })
      .eq('id', dist.id);
    if (!error) {
      setDistributors(prev => prev.map(d => d.id === dist.id ? {
        ...d, status:'approved', tier: form.tier||'entry', login_code: form.login_code.toUpperCase()
      } : d));
    }
  ...
  if (loading) return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'50vh', color:'#8C919E' }}>
      <div style={{ textAlign:'center' }}><div style={{ fontSize:32, marginBottom:8 }}>🏭</div>Loading distributors...</div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <p style={{ color:'#8C919E', fontSize:14, marginBottom:24 }}>
        {distributors.length} applications · {approved} approved · {pending} pending review
      </p>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:24 }}>
        {[
          ...
          { l:'Total Dist. Revenue',v: '$'+totalSpent.toFixed(2), c:'#0072B5' },
        ].map((x,i) => (
      ...
      <div style={{ display:'flex', gap:12, marginBottom:16, flexWrap:'wrap' }}>
        ...
        <div style={{ display:'flex', gap:4 }}>
          {['all', ...Object.keys(STATUS_COLOR)].map(s => (
            ...
            }}>{s==='all' ? 'All' : STATUS_COLOR[s].label}</button>
          ))}
        </div>
      </div>
        ...
        {filtered.map(dist => {
          ...
          return (
            <div key={dist.id} style={{ ...cs.card, border: dist.status==='pending' ? '1px solid #FCD34D' : '1px solid #E4E7EC' }}>
              <div onClick={() => setExpanded(ie ? null : dist.id)}
                style={{ padding:'16px 20px', cursor:'pointer', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <div style={{ display:'flex', gap:20, alignItems:'center' }}>
                  <div style={{ width:36, height:36, borderRadius:8, background:'#F0FDF4', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700, fontSize:14, color:'#22C55E', flexShrink:0 }}>
                    {(dist.business_name||'?').charAt(0).toUpperCase()}
                  </div>
                  <div>
                    <div style={{ fontSize:13, fontWeight:600, color:'#0F1928' }}>{dist.business_name}</div>
                    <div style={{ fontSize:11, color:'#8C919E' }}>{dist.contact_name} · {dist.email}</div>
                  </div>
                  <span style={{ fontSize:10, color:'#8C919E', background:'#F7F8FA', padding:'2px 8px', borderRadius:4 }}>{dist.market}</span>
                  {isApproved && dist.tier && (
                    <span style={{ fontSize:10, fontWeight:600, background:'#EFF6FF', color:'#0072B5', padding:'2px 8px', borderRadius:4 }}>
                      {dist.tier.charAt(0).toUpperCase()+dist.tier.slice(1)} · {TIER_DISC[dist.tier]}
                    </span>
                  )}
                </div>
                <div style={{ display:'flex', gap:16, alignItems:'center' }}>
                  {isApproved && (
                    <div style={{ textAlign:'right' }}>
                      <div style={{ fontFamily:"'JetBrains Mono'", fontSize:13, fontWeight:700, color:'#0072B5' }}>${myRevenue.toFixed(2)}</div>
                      <div style={{ fontSize:11, color:'#8C919E' }}>{myOrders.length} orders</div>
                    </div>
                  )}
                ...
                <div style={{ padding:'0 20px 20px', borderTop:'1px solid #F0F1F4' }}>
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:24, marginTop:16 }}>
                    ...
                    <div>
                      ...
                      {[
                        { l:'Business',        v: dist.business_name },
                        { l:'Contact',         v: dist.contact_name },
                        { l:'Email',           v: dist.email },
                        { l:'Phone',           v: dist.phone },
                        { l:'Market',          v: dist.market },
                        { l:'Expected Volume', v: `${dist.expected_volume}+ vials/mo` },
                        { l:'Website',         v: dist.website || '—' },
                      ].map((x,i) => (
                        <div key={i} style={{ display:'flex', gap:12, padding:'6px 0', borderBottom:'1px solid #F0F1F4', fontSize:13 }}>
                          <span style={{ color:'#8C919E', width:120, flexShrink:0 }}>{x.l}</span>
                          <span style={{ color:'#0F1928' }}>{x.v}</span>
                        </div>
                      ))}
                      {dist.notes && (
                        <div style={{ marginTop:12, padding:'10px 12px', background:'#F7F8FA', borderRadius:4, fontSize:13, color:'#4A4F5C', lineHeight:1.7 }}>
                    ...
                    <div>
                      {!isApproved ? (
                        <>
                          ...
                          <div style={{ display:'flex', flexDirection:'column', gap:10, marginBottom:16 }}>
                            <div>
                              ...
                              <input
                                style={{ ...cs.input, width:'100%', textTransform:'uppercase' }}
                                placeholder="e.g. ACME2026"
                                value={form.login_code || ''}
                                onChange={e => setApproveForm(prev => ({ ...prev, [dist.id]: { ...form, login_code: e.target.value.toUpperCase() } }))}
                              />
                            </div>
                            <div>
                              ...
                              <select
                                ...
                                value={form.tier || 'entry'}
                                onChange={e => setApproveForm(prev => ({ ...prev, [dist.id]: { ...form, tier: e.target.value } }))}
                              >
                          ...
                          <button
                            onClick={() => approve(dist)}
                            disabled={saving[dist.id]}
                            style={{ ...cs.btn, background:'#22C55E', color:'#fff', width:'100%', marginBottom:8, opacity: saving[dist.id] ? 0.5 : 1 }}
                          >
                            {saving[dist.id] ? 'Saving...' : '✓ Approve & Save to Supabase'}
                          </button>
                          <button
                            onClick={() => updateStatus(dist.id, 'rejected')}
                            style={{ ...cs.btn, background:'#FEF2F2', color:'#EF4444', width:'100%', border:'1px solid #FCA5A5' }}
                          >
                            ✗ Reject Application
                          </button>
                        </>
                        ...
                        <>
                          ...
                          {[
                            { l:'Login Code',  v: dist.login_code || '—' },
                            { l:'Tier',        v: dist.tier ? dist.tier.charAt(0).toUpperCase()+dist.tier.slice(1) : '—' },
                            { l:'Discount',    v: TIER_DISC[dist.tier] || '—' },
                            { l:'Approved',    v: dist.approved_at ? new Date(dist.approved_at).toLocaleDateString() : '—' },
                            { l:'Total Orders',v: `${dist.total_orders||0}` },
                            { l:'Total Spent', v: `$${parseFloat(dist.total_spent||0).toFixed(2)}` },
                          ].map((x,i) => (
                            <div key={i} style={{ display:'flex', gap:12, padding:'6px 0', borderBottom:'1px solid #F0F1F4', fontSize:13 }}>
                              <span style={{ color:'#8C919E', width:110, flexShrink:0 }}>{x.l}</span>
                              <span style={{ color:'#0F1928', fontFamily: x.l==='Login Code'?"'JetBrains Mono'":'inherit', fontWeight: x.l==='Login Code'?600:400 }}>{x.v}</span>
                            </div>
                          ))}

                          <button
                            onClick={() => sendApprovalEmail(dist)}
                            disabled={sendingApproval[dist.id]}
                            style={{ ...cs.btn, background:'#0072B5', color:'#fff', width:'100%', marginTop:16, opacity: sendingApproval[dist.id] ? 0.5 : 1 }}
                          >
                            ...
                            <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                              {Object.entries(TIER_DISC).map(([k,v]) => (
                                <button key={k} onClick={async () => {
                                  await supabase.from('distributors').update({ tier: k }).eq('id', dist.id);
                                  setDistributors(prev => prev.map(d => d.id===dist.id ? {...d, tier:k} : d));
                                }} style={{
                                  ...cs.btn, padding:'6px 12px', fontSize:11,
                                  background: dist.tier===k ? '#0072B5' : '#F7F8FA',
                                  color:      dist.tier===k ? '#fff'    : '#6B7A94',
                                  border: '1px solid '+(dist.tier===k ? '#0072B5' : '#E4E7EC'),
                          ...
                          <button
                            ...
                            style={{ ...cs.btn, background:'#F7F8FA', color:'#8C919E', width:'100%', marginTop:12, border:'1px solid #E4E7EC' }}
                          >
                            Suspend Account
                          </button>
                        </>
