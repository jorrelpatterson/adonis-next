...
export default function OrdersPage() {
  ...
  const updateStatus = async (orderId, newStatus) => {
    const { error } = await supabase.from('orders')
      ...
      .eq('order_id', orderId);
    if (!error) setOrders(prev => prev.map(o => o.order_id === orderId ? { ...o, status: newStatus } : o));
  };
  ...
  const sendShipping = async (order) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert('Shipping confirmation sent to ' + order.email);
      } else {
        alert('Email failed — check Resend logs');
      }
    } catch(e) {
  ...
  if (loading) return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'50vh', color:'#8C919E' }}>
      <div style={{ textAlign:'center' }}><div style={{ fontSize:32, marginBottom:8 }}>🛒</div>Loading orders...</div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <p style={{ color:'#8C919E', fontSize:14, marginBottom:24 }}>
        {orders.length} orders · ${totalRevenue.toLocaleString('en-US',{minimumFractionDigits:2})} revenue · <span style={{color:'#22C55E'}}>Live</span>
      </p>
          ...
          {filtered.map(order => {
            ...
            return (
              <div key={order.order_id} style={{ ...cs.card, border: order.price_mismatch ? '1px solid #FCA5A5' : '1px solid #E4E7EC' }}>
                <div onClick={() => setExpanded(ie ? null : order.order_id)}
                  style={{ padding:'16px 20px', cursor:'pointer', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  <div style={{ display:'flex', gap:20, alignItems:'center' }}>
                    <div style={{ fontFamily:"'JetBrains Mono'", fontSize:12, fontWeight:500, color:'#0072B5', minWidth:160 }}>
                      {order.order_id}
                    </div>
                    <div>
                      <div style={{ fontSize:13, fontWeight:600, color:'#0F1928' }}>{customerName}</div>
                      <div style={{ fontSize:11, color:'#8C919E' }}>{order.email}</div>
                    </div>
                    {order.price_mismatch && (
                      <span style={{ fontSize:10, background:'#FEF2F2', color:'#EF4444', padding:'2px 8px', borderRadius:4, fontWeight:600 }}>⚠ Price mismatch</span>
                    )}
                    {order.ref_code && (
                      <span style={{ fontSize:10, background:'#EFF6FF', color:'#0072B5', padding:'2px 8px', borderRadius:4 }}>ref: {order.ref_code}</span>
                    )}
                  </div>
                  ...
                  <div style={{ padding:'0 20px 20px', borderTop:'1px solid #F0F1F4' }}>
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:24, marginTop:16 }}>
                      ...
                      <div>
                        ...
                        {items.map((item,i) => (
                          <div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'6px 0', borderBottom:'1px solid #F0F1F4', fontSize:13 }}>
                            ...
                            <span style={{ fontFamily:"'JetBrains Mono'", fontSize:12, fontWeight:500 }}>${(item.price*item.qty).toFixed(2)}</span>
                          </div>
                        ))}
                        ...
                        {order.notes && (
                          <div style={{ marginTop:12, padding:'8px 12px', background:'#FFFBEB', borderRadius:4, fontSize:12, color:'#78716C' }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize:13, color:'#4A4F5C', lineHeight:1.8, marginBottom:20 }}>
                          {customerName}<br/>
                          {order.address}<br/>
                        ...
                        <div style={{ display:'flex', gap:8, marginBottom:16 }}>
                          ...
                          />
                          <button
                            onClick={() => sendShipping(order)}
                            disabled={sending[order.order_id]}
                        ...
                        <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                          {Object.entries(STATUS).map(([k, cfg]) => (
                            <button key={k} onClick={() => updateStatus(order.order_id, k)} style={{
                              ...cs.btn, padding:'6px 12px', fontSize:11,
