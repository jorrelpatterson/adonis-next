...
// Groups the fine-grained `cat` field into 10 buckets: Peptide, Weight Loss, Steroid, SARM, PCT, Thyroid, Stimulant, Pharma, Supply, Cosmetic.
export const PRODUCT_TYPES = ['Peptide', 'Weight Loss', 'Steroid', 'SARM', 'PCT', 'Thyroid', 'Stimulant', 'Pharma', 'Supply', 'Cosmetic'];

export function typeFor(cat) {
  if (!cat) return 'Peptide';
  const c = String(cat).trim();
  if (c === 'Anabolic') return 'Steroid';
  if (c === 'SARM') return 'SARM';
  if (c === 'PCT') return 'PCT';
  if (c === 'Thyroid') return 'Thyroid';
  if (c === 'Stimulant') return 'Stimulant';
  if (c === 'Pharma') return 'Pharma';
  if (c === 'Supplies') return 'Supply';
  if (c === 'Cosmetic') return 'Cosmetic';
  if (c === 'Weight Loss') return 'Weight Loss';
  // Remaining cats (Recovery, Growth Hormone, Longevity, Immune, Hormonal, Cognitive, Skin, Sleep, Metabolic) → Peptide
  return 'Peptide';
}
