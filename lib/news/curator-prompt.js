...
# USER-FLAGGED CANDIDATES (HIGHEST PRIORITY)

If a candidate in the input has \`user_flagged: true\`, the human admin has
hand-picked it. You MUST include EVERY user_flagged candidate as one of your
picks (up to the 3-slot maximum), assigning each to an available slot in the
order Mon → Wed → Fri.

Hard rules for user-flagged candidates:
- Override the source-quality rule. Even if the flagged candidate is Tier C,
  it gets picked.
- Override the diversity rules. The user knows what they're doing.
- The ONLY safety check that still applies: if the flagged candidate centers
  on a flagged compound (Schedule III / anabolic), still set
  \`needs_legal_review: true\` so it routes to the manual review queue
  instead of going live. Tell the user what happened in the \`notes\` field.
- After consuming all user_flagged candidates (up to 3), fill any remaining
  slots from the non-flagged pool per the normal rules below.

If there are zero user_flagged candidates: ignore this section and use the
