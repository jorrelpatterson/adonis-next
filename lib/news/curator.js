...
async function fetchUnpickedCandidates() {
  // Order flagged-first so the curator sees user-flagged candidates at the top
  // of the list. Flagged ones are also passed with user_flagged: true in the
  // user message so the prompt's must-include rule kicks in.
  const res = await sb('/news_candidates?status=eq.new&order=flagged_for_curate.desc,flagged_at.desc.nullslast,published_at.desc&limit=200');
  return res.json();
}
...
export async function runCurator() {
  ...
  // Limit user_flagged to 3 (the slot count). If user flagged more than 3,
  // only the most-recently-flagged 3 get the must-include treatment;
  // the rest still appear in the list but as user_flagged: false this week.
  const flaggedKeep = new Set(
    candidates.filter((c) => c.flagged_for_curate).slice(0, 3).map((c) => c.id),
  );
  ...
  const userMessage = JSON.stringify({
    ...
    candidates: candidates.map((c) => ({
      ...
      user_flagged: flaggedKeep.has(c.id),
    })),
  });
  ...
  for (const pick of parsed.picks) {
    ...
    // Clear flag at the same time we mark picked, so a manually-flagged
    // candidate doesn't keep getting re-picked week after week.
    await sb(`/news_candidates?id=eq.${pick.candidate_id}`, {
      method: 'PATCH',
      body: JSON.stringify({ status: 'picked', flagged_for_curate: false, flagged_at: null }),
    });
  }
