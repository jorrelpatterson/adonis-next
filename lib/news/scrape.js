...
async function insertCandidates(items) {
  ...
  // PostgREST needs an explicit ?on_conflict=<col> for ignore-duplicates to
  // trap unique violations. Without it, a single conflict in the batch raises
  // 409. With it, conflicting rows are silently skipped per ON CONFLICT DO NOTHING.
  const res = await sb(`/news_candidates?on_conflict=source_url`, {
    method: 'POST',
    headers: {
