...
export async function renderInvoicePng(inv) {
  ...
  // Compute height based on content. Without this, ImageResponse defaults
  // to 1200×630 (OG-image size) and clips everything below the fold.
  // Base height covers header + title + bill-to + totals + Zelle + link + footer.
  // Each item adds one row; a visible discount adds one totals row.
  const hasDiscount = inv.discount_applied_cents > 0;
  const height = 880 + (inv.items?.length || 0) * 90 + (hasDiscount ? 38 : 0);
